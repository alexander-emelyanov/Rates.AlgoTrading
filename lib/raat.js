...
var ReceivedTicksProcessor = require('./processors/ReceivedTicksProcessor');
...
function Raat(configuration){
    ...
    // @TODO: Processors fabric will be here...
    var receivedTicksProcessor = new ReceivedTicksProcessor();

    // Routes loading
    var ticksRoute = require('./routes/ticks');
    ...
    sockjsServer.on('connection', function(connection) {
        ...
        connection.on('data', function(message) {
            try{
                ...
                // @TODO Refactor this!
                // Temporary code, must be refactored
                if (messageObject.type && messageObject.type == 'tick'){
                    receivedTicksProcessor.process(messageObject);
                }

                redisClient.publish(channel, JSON.stringify(messageObject));
            } catch (e){
                console.log("RAAT: Invalid message JSON format", message, e);
            }
    ...
    expressApplication.use('/ticks', ticksRoute);
